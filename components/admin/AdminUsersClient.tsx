    ...
    <div style={{ padding: "clamp(16px, 4vw, 36px)" }}>
      ...
      {toast && (
        <div style={{
          ...
          background: toast.ok ? "var(--bg-card)" : "var(--bg-card)",
          border: `1.5px solid ${toast.ok ? "#22c55e" : "#ef4444"}`,
          borderRadius: 11, padding: "11px 18px", fontSize: 13,
          color: toast.ok ? "#86efac" : "#fca5a5", fontWeight: 700,
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
        <div>
          <div style={{ fontSize: 10, color: "var(--accent)", fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 5 }}>Admin</div>
          <h1 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 40, letterSpacing: ".05em", lineHeight: 1, color: "var(--text)" }}>USER MANAGEMENT</h1>
          <div style={{ fontSize: 13, color: "var(--text-muted)", marginTop: 6 }}>{users.length} users · manage access levels and permissions</div>
        </div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 24, flexWrap: "wrap" }}>
        {ROLES.map(r => (
          <div key={r.value} style={{ background: "var(--bg-card)", border: `1.5px solid ${r.color}25`, borderRadius: 10, padding: "10px 16px", display: "flex", alignItems: "flex-start", gap: 10, flex: "1 1 180px" }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2, lineHeight: 1.4 }}>{r.desc}</div>
            </div>
          </div>
        ))}
      </div>
          ...
          <thead>
            <tr style={{ background: "var(--bg-deep)" }}>
              {["User", "Role", "Status", "Joined", "Actions"].map(h => (
                <th key={h} style={{ padding: "12px 18px", textAlign: "left", fontSize: 10, fontWeight: 700, color: "var(--text-dim)", letterSpacing: ".07em", textTransform: "uppercase", borderBottom: "1.5px solid var(--border)" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {users.map(u => {
              ...
              const protected_ = isOwner && !isSelf;
              return (
              <tr key={u.id} style={{ borderBottom: "1px solid var(--border)" }}>
                <td style={{ padding: "14px 18px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <span style={{ fontSize: 13, fontWeight: 700, color: "var(--text)" }}>{u.full_name || "—"}</span>
                    {isOwner && (
                      <span title="Workspace Owner" style={{ color: "#f59e0b", display: "flex", alignItems: "center" }}>
                        <Crown size={13} />
                      </span>
                    )}
                  </div>
                  <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>{u.email}</div>
                  {isSelf && <span style={{ fontSize: 10, color: "var(--accent)", fontWeight: 700 }}>YOU</span>}
                </td>
                <td style={{ padding: "14px 18px" }}>
                  {isSelf || protected_ ? (
                ...
                <td style={{ padding: "14px 18px" }}>
                  {!isSelf && (
                    protected_ ? (
                      <span style={{ fontSize: 11, color: "var(--text-dim)", fontStyle: "italic" }}>Protected</span>
                    ) : (
      ...
      {showInvite && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ background: "var(--bg-card)", border: "1.5px solid var(--border-lt)", borderRadius: 16, padding: 32, width: "100%", maxWidth: 460, boxShadow: "var(--shadow)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
              <h2 style={{ fontSize: 20, fontWeight: 800, color: "var(--text)" }}>Invite New User</h2>
              <button onClick={() => { setShowInvite(false); setInviteError(""); }} style={{ background: "none", border: "none", color: "var(--text-muted)", fontSize: 20, cursor: "pointer" }}>✕</button>
            </div>
            <form onSubmit={handleInvite} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text-muted)", letterSpacing: ".07em", textTransform: "uppercase", marginBottom: 6 }}>Full Name</div>
                <input className="dark-input" placeholder="Jane Smith" value={inviteData.full_name} onChange={e => setInviteData(p => ({ ...p, full_name: e.target.value }))} required />
              </div>
              <div>
                <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text-muted)", letterSpacing: ".07em", textTransform: "uppercase", marginBottom: 6 }}>Email</div>
                <input className="dark-input" type="email" placeholder="jane@company.com" value={inviteData.email} onChange={e => setInviteData(p => ({ ...p, email: e.target.value }))} required />
              </div>
              <div>
                <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text-muted)", letterSpacing: ".07em", textTransform: "uppercase", marginBottom: 6 }}>Temporary Password</div>
                <input className="dark-input" type="password" placeholder="Min. 8 characters" value={inviteData.password} onChange={e => setInviteData(p => ({ ...p, password: e.target.value }))} required minLength={8} />
              </div>
              <div>
                <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text-muted)", letterSpacing: ".07em", textTransform: "uppercase", marginBottom: 8 }}>Access Level</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {ROLES.map(r => (
                    <label key={r.value} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderRadius: 9, border: `1.5px solid ${inviteData.role === r.value ? r.color : "var(--border-lt)"}`, background: inviteData.role === r.value ? `${r.color}10` : "transparent", cursor: "pointer", transition: "all .2s" }}>
                      ...
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 700, color: inviteData.role === r.value ? r.color : "var(--text)" }}>{r.label}</div>
